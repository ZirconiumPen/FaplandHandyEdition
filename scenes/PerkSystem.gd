...
func check_perk_rewards(current_round: int):
	...
	if (current_round >= 25 and not c1) or (current_round >= 50 and not c2) or (current_round >= 75 and not c3):
		if current_round>=25:
			c1=true
		if current_round>=50:
			c2=true
...
func add_perk(perk_id: String):
	...
	if perks_inventory.size() >= max_perks_inventory:
		...
		main_game.show_aaa_popup("⚠️ Perk inventory full! Lost: " + perk_data[removed_perk].name, Color.ORANGE)
...
func show_perk_selection_popup():
	...
	for i in range(perks_inventory.size()):
		...
		button_style.bg_color = Color(perk.color.r * 0.3, perk.color.g * 0.3, perk.color.b * 0.3, 0.9)
...
func use_perk(perk_id: String, inventory_index: int):
	...
	if (perk_id=="skip_round" or perk_id=="reroll_dice") and main_game.ui_elements.has("roll_button") and not main_game.ui_elements["roll_button"].disabled:
		main_game.show_aaa_popup("❌ Roll the dice first!", Color.RED)
		close_perk_popup()
		return
		
	# Check if Lucky 7 is being used after dice have been rolled
	if perk_id == "lucky_7" and main_game.ui_elements.has("roll_button") and main_game.ui_elements["roll_button"].disabled:
...
func apply_reroll_dice():
	...
	if main_game.ui_elements.has("roll_button"):
		...
		activate_tween.tween_property(main_game.ui_elements["roll_button"], "modulate", Color.WHITE, 0.4)
		activate_tween.tween_property(main_game.ui_elements["roll_button"], "scale", Vector2(1.08, 1.08), 0.3)
		activate_tween.tween_property(main_game.ui_elements["roll_button"], "scale", Vector2(1.0, 1.0), 0.3)
		
		# Add premium glow effect
		var glow_tween = main_game.create_tween()
		glow_tween.set_loops()
		glow_tween.tween_property(main_game.ui_elements["roll_button"], "modulate", Color(1.3, 1.3, 1.3, 1.0), 1.2)
		glow_tween.tween_property(main_game.ui_elements["roll_button"], "modulate", Color.WHITE, 1.2)

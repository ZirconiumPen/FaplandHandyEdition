...
def create_sprite_sheet(frames, output_path):
    ...
    try:
        ...
        print(f"Created sprite sheet: {os.path.basename(output_path)} ({frames_per_row}x{frames_per_col} grid)")
        return frame_width, frame_height, frames_per_row, frames_per_col
    except Exception as e:
        print(f"Error saving sprite sheet {output_path}: {e}")
        return None, None, None, None

def create_tres_content(sprite_sheet_path, frame_durations, frame_width, frame_height, 
                       frames_per_row, frames_per_col, total_frames):
...
def process_gif_to_tres(gif_num):
    ...
    frame_width, frame_height, frames_per_row, frames_per_col = create_sprite_sheet(frames, sprite_sheet_path)
    ...
    tres_content = create_tres_content(
        relative_sprite_path, durations, frame_width, frame_height,
        frames_per_row, frames_per_col, len(frames)
    )
...
def main():
    ...
    print(f"\n" + "=" * 50)
    print(f"Conversion complete! Successfully processed {successful_conversions} GIF files.")
    print(f"\nGenerated files:")
    print(f"- Sprite sheets: {SPRITE_SHEET_DIR}/")
    print(f"- TRES resources: {TRES_DIR}/")
    
    print(f"\nTo use in Godot:")
    print(f"1. Copy the '{SPRITE_SHEET_DIR}' and '{TRES_DIR}' folders to your Godot project")

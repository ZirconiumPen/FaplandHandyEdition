...
def load_pause_config():
    ...
    try:
        ...
        if "entries" in pause_data and pause_data["entries"]:
            ...
            logger.info(f"🔍 DEBUG: Found {len(pause_data['entries'])} entries in pause config")
            ...
            for i, entry in enumerate(sorted(pause_data["entries"], key=lambda x: x["timestamp"])):
                logger.info(f"  {i+1}. {entry['timestamp']} | {entry['writer']} | pauses={entry['max_pauses']} | reason={entry.get('reason', 'unknown')}")
        ...
        logger.info(f"📝 Loaded pause config: {max_pauses} max, {pause_duration}s duration, {pauses_used} used")
...
def save_pause_config(reason="unknown"):
    ...
    try:
        ...
        logger.info(f"🔍 DEBUG: About to save - max_pauses={max_pauses}, pauses_used={pauses_used}, reason={reason}")
...
def log_system_info():
    ...
    try:
        ...
        try:
            version_info = vlc.libvlc_get_version().decode() if hasattr(vlc, 'libvlc_get_version') else 'Unknown'
...
def get_server_time():
    ...
    try:
        ...
        resp = requests.get(f"https://www.handyfeeling.com/api/handy/v2/servertime", timeout=10)
...
def setup_hssp(script_url):
    ...
    try:
        ...
        logger.debug(f"Mode response: {mode_response.status_code} - {mode_response.text}")
        ...
        logger.debug(f"HSSP setup response: {setup_response.status_code} - {setup_response.text}")
...
def sync_time_hssp(headers, video_ms):
    ...
    try:
        ...
        sync_resp = requests.put(sync_url, headers=headers, json=sync_payload, timeout=5)
        
        logger.debug(f"HSSP synctime response: {sync_resp.status_code}")
        sync_resp.raise_for_status()
        
    except Exception as e:
        logger.debug(f"Non-critical sync time error: {e}")  # Don't fail the whole session for sync errors
    ...
    for attempt in range(max_attempts):
        try:
            ...
            try:
                ...
                logger.debug(f"Attempt {attempt + 1}: Fullscreen status: {is_fullscreen}")
                if is_fullscreen:
                    logger.info(f"✅ Successfully entered fullscreen on attempt {attempt + 1}")
                    return True
            except Exception as e:
...
def monitor_keyboard(player, headers=None):
    ...
    global max_pauses,pause_duration,pauses_used,original_max_pauses
    try:
        logger.info(f"⏸️ Keyboard monitor started - Pauses: {max_pauses}x{pause_duration}s available")
        ...
        def update_vlc_overlay(message, temporary=False):
            try:
                if message:  # Show overlay
                    ...
                    player.video_set_marquee_string(vlc.VideoMarqueeOption.Text, message.encode('utf-8'))
                    player.video_set_marquee_int(vlc.VideoMarqueeOption.Position, vlc.Position.TopLeft)
                    player.video_set_marquee_int(vlc.VideoMarqueeOption.Size, 16)  # Smaller text
                    player.video_set_marquee_int(vlc.VideoMarqueeOption.Color, 0x00FF00)
                    player.video_set_marquee_int(vlc.VideoMarqueeOption.Opacity, 200)  # Slightly transparent
                ...
                # Check for pause key (SPACE) - only if not currently in a pause cycle
                    ...
                    if state == vlc.State.Playing:  # Only pause if actually playing
                        ...
                        if available_pauses > 0:
                            ...
                            logger.info(f"⏸️ PAUSE {pauses_used}/{original_max_pauses} - Starting {pause_duration}s timer")
...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.error("   1. Download and install VLC: https://www.videolan.org/vlc/")
        ...
        if len(sys.argv) < 2:
            ...
            logger.error("Usage: python sync_handy.py <video_name> [max_pauses] [pause_duration]")
        ...
        for i, vlc_args in enumerate(vlc_configs):
            try:
                ...
                try:
                    player.set_fullscreen(True)
                    logger.debug(f"✅ Configuration {i + 1}: Fullscreen set successfully")
                except Exception as fs_error:
                    logger.debug(f"⚠️ Configuration {i + 1}: Could not set fullscreen: {fs_error}")
                
                logger.info(f"✅ VLC initialized successfully with configuration {i + 1}")
                break
        ...
        if instance is None or player is None:
            ...
            logger.error("   1. Make sure VLC is installed: https://www.videolan.org/vlc/")
        ...
        while True:
            ...
            if loop_count < 120:  # First 30 seconds (120 * 0.25s)
                if loop_count % 4 == 0:  # Every 1 second
                    try:
                        if not player.get_fullscreen():
                            logger.debug(f"🖥️ Loop {loop_count}: Not fullscreen, fixing...")
                            player.set_fullscreen(True)
                    except Exception as e:
                ...
                if loop_count % 40 == 0:  # Every 10 seconds
                    logger.debug(f"Player state: {state}, Position: {player.get_position()}")
            ...
            if was_playing and state == vlc.State.Playing and sync_start_time is not None:
                video_time = player.get_time()
                if video_time is not None and video_time >= 0:

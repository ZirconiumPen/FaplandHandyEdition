...
func animate_particle(particle: Label):
	...
	var target_pos = Vector2(
		start_pos.x + randf_range(-100, 100),
		start_pos.y + randf_range(-50, 50)
	)
...
func _on_install_deps_pressed():
	...
	if not python_found:
		show_premium_popup("❌ Python not found! Please install Python first from python.org", Color.RED)
		return
...
func play_entrance_animation():
	...
	if ui_elements.has("title_container"):
		...
		title_tween.parallel().tween_property(title_container, "position:y", title_container.position.y + 100, 1.0)
...
func create_highscore_panel(parent: Control):
	...
	best_score_label.text = "Best: Round " + str(best_round) if best_round > 0 else "Best: No scores yet"
	...
	for i in range(min(6, highscores.size())):
		...
		# Color based on rank
		var score_color = Color.GOLD if i == 0 else Color.SILVER if i == 1 else Color(0.8, 0.5, 0.2, 1.0) if i == 2 else Color.WHITE
